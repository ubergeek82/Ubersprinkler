...
typedef struct weather_forecast_response_t {
	...
	bool cached;
	// defaults:
	weather_forecast_response_t(): temp_high(255), temp_low(255), conditionCode(-1), isSuccess(false) {};
} weather_forecast_response_t;

class Weather {
public:
	Weather(String location, HttpClient* client, String apiKey);
	bool updateForecast(weather_forecast_response_t& response);
	void setCelsius();
	void setFahrenheit();

	// for cache:
	weather_forecast_response_t cachedUpdate();
...
private:
	...
	HttpClient* client;
	bool parse(String& json, weather_forecast_response_t& response);
